...
graduation<-graduation %>% mutate(`Total Grads - % of cohort`=parse_number(`Total Grads - % of cohort`),`Dropped Out - % of cohort` = parse_number(`Dropped Out - % of cohort`))

## 4. Extracting Numeric Data From Strings: Creating New Variables ##

split_location<-str_split(hs_directory$`Location 1`,"\n",simplify=TRUE)
hs_directory<-hs_directory %>% mutate(lat_long=split_location[,3])

## 5. Splitting Strings ##

lat_and_long<-str_split(hs_directory$lat_long,",",simplify=TRUE)
hs_directory<-hs_directory %>% mutate(lat=lat_and_long[,1],long=lat_and_long[,2])
